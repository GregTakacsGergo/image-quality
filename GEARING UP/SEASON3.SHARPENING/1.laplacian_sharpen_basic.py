'''SEASON3.SHARPENING / 1.laplacian_sharpen_basic.py

Pure Laplacian sharpening — no pre-blur.
Formula: sharpened = original - k * Laplacian(original)

The Laplacian detects edges (second derivative). Subtracting it from the
original amplifies those edges, making the image appear sharper.

Controls:
  k slider  — sharpening aggressiveness (0 = no change, 5 = heavy sharpening)
  Sharpen   — apply sharpening and display before/after
  Save      — write the sharpened image to the output folder
'''
...
OUTPUT_DIR = os.path.join(SEASON_DIR, "output", "laplacian_basic")
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        filename=os.path.join(LOG_DIR, "1.laplacian_sharpen_basic.log"),
        filemode="w",
    )
...
def laplacian_sharpen(image_bgr: np.ndarray, k: float) -> np.ndarray:
    """Return sharpened image using: sharpened = image - k * Laplacian(image)."""
...
class LaplacianSharpenApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Laplacian Sharpening — Basic (Season 3)")

        self.image_cv      = None   # original image (BGR)
        self.sharpened_cv  = None   # last sharpened result (BGR)
        self.image_path    = ""
        ...
        self._build_ui()
        logging.info("LaplacianSharpenApp started.")
    ...
    def _build_canvases(self):
        ...
        tk.Label(right, text="Sharpened", bg="#222", fg="#aaa",
                 font=("Helvetica", 11, "bold")).pack()
    ...
    def _build_controls(self):
        ...
        # k slider
        tk.Label(ctrl, text="Sharpening strength  k:", bg="#2a2a2a",
                 fg="white").grid(row=0, column=0, padx=10, sticky="w")
        self.k_slider = tk.Scale(
            ctrl, from_=K_MIN, to=K_MAX, resolution=K_STEP,
            orient="horizontal", variable=self.k_var, length=300,
            bg="#2a2a2a", fg="white", highlightthickness=0,
            troughcolor="#555",
        )
        self.k_slider.grid(row=0, column=1, padx=10)
        self.k_value_label = tk.Label(ctrl, textvariable=self.k_var,
                                      bg="#2a2a2a", fg="#7fc", width=5)
        self.k_value_label.grid(row=0, column=2)
    ...
    def _apply_sharpening(self):
        ...
        k = self.k_var.get()
        logging.info(f"Applying Laplacian sharpening with k={k:.2f}")

        self.sharpened_cv = laplacian_sharpen(self.image_cv, k)
        ...
        self.status_var.set(f"Sharpened with k={k:.2f}  |  "
    ...
    def _save_result(self):
        ...
        fname  = f"{base}_laplacian_k{k_str}.png"
